...
class AnimalPanel(wx.Panel):
  ...
  def __init__(self, notebook, animaldata, clientpanel=False):
    ...
    speciesentry.Bind(wx.EVT_CHAR, self.OnSpeciesChange)
    speciesentry.Bind(wx.EVT_COMBOBOX, self.OnSpeciesChange)
  ...
  def OnSpeciesChange(self, ev):
    self.UpdateBreedField(ev.GetEventObject().GetValue())
    
    self.EnableSave(ev)
  
  def UpdateBreedField(self, species):
    self.breedentry.Clear()
    
    action = "SELECT DISTINCT(BreedName) FROM breed WHERE species IS NULL OR species = '"+species+"' OR species = '';"
    results = db.SendSQL(action, self.localsettings.dbconnection)
    
    if len(results) != 0:
      for a in results:
        self.breedentry.Append(a[0])
  ...
  def GetLookupsList(self, table, extra = None):
    
    columnname = table[0].upper() + table[1:] + "Name"
    
    if table == 'breed' and extra != None:
      action = "SELECT DISTINCT(" + columnname + ") FROM " + table + " WHERE species IS NULL OR species = '" + extra + "' OR species = '' ORDER BY " + columnname
    else:
