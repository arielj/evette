...
class EditLookup(wx.Panel):
  ...
  def __init__(self, notebook, lookup, localsettings):
    ...
    self.pagetitle = miscmethods.GetPageTitle(notebook, self.t("editlabel")+" "+self.t("lookups"+lookup))
  ...
  def Popup(self, ID):
    ...
    add = wx.MenuItem(popupmenu, ADD_LOOKUP, self.t("addlabel")+" "+self.t("lookup"+self.lookup))
    ...
    if self.listbox.GetSelection() > -1:
      ...
      edit = wx.MenuItem(popupmenu, EDIT_LOOKUP, self.t("editlabel")+" "+self.t("lookup"+self.lookup))
      edit.SetBitmap(wx.Bitmap("icons/edit.png"))
      popupmenu.AppendItem(edit)
      wx.EVT_MENU(popupmenu, EDIT_LOOKUP, self.EditLookup)
      
      delete = wx.MenuItem(popupmenu, DELETE_LOOKUP, self.t("deletelabel")+" "+self.t("lookup"+self.lookup))
  ...
  def EditLookupDialog(self, lookupid=False):
    ...
    t_key = 'edit' if lookupid else 'add'
    t = self.t(t_key+'label')+" "
    t_key = "lookup"+self.lookup
    t += self.t(t_key)
    
    dialog = wx.Dialog(self, -1, t)
